...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/02/12 00:00:47 by ymarival          #+#    #+#             */
/*   Updated: 2023/02/12 20:38:26 by ymarival         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int ac, char **av, char **env)
{
	...
	if (ac >= 5)
	{
		...
		outfile_fd = openfile(av[ac - 1], 1);
		dup2(infile_fd, 0);
		dup2(outfile_fd, 1);
		while (i < ac - 2)
		{
			first_command_processing(av[i++], env, infile_fd);
		}
		second_command_processing(av[i], env);
	}
	else
		write(2, "Need more than 4 arguments\n", 18);
	return (1);
}
